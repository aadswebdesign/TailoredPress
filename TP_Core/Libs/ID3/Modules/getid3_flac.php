    ...
    class getid3_flac extends getid3_handler{
       public const SYNC_WORD = 'fLaC';
        public function Analyze():bool {
            ...
            if ($StreamMarker !== self::SYNC_WORD) {
                return $this->error('Expecting "'.getid3_lib::PrintHexBytes(self::SYNC_WORD).'" at offset '.$info['avdataoffset'].', found "'.getid3_lib::PrintHexBytes($StreamMarker).'"');
            }
        ...
        public function parseMETAdata():bool {
            ...
            do {
                ...
                $LBFBT         = getid3_lib::BigEndian2Int($BlockHeader[0]);  // LBFBT = LastBlockFlag + BlockType
        ...
        public static function parseSTREAMINFOdata($BlockData):array {
            $streaminfo = array();
        ...
        private function parseSTREAMINFO($BlockData):bool {
            $info = &$this->getid3->info;
        ...
        private function parseAPPLICATION($BlockData):bool {
            $info = &$this->getid3->info;
        ...
        private function parseSEEKTABLE($BlockData):bool {
            $info = &$this->getid3->info;
            $offset = 0;
        ...
        private function parseVORBIS_COMMENT($BlockData):bool {
            $info = &$this->getid3->info[$BlockData];//this way ?
        ...
        private function parseCUESHEET($BlockData):bool {
            $info = &$this->getid3->info;
            $offset = 0;
            ...
            $offset += 8;
            $info['flac']['CUESHEET']['flags']['is_cd'] = (bool) (getid3_lib::BigEndian2Int($BlockData[$offset]) & 0x80);
            ++$offset;
            $offset += 258; // reserved
            $info['flac']['CUESHEET']['number_tracks'] =  getid3_lib::BigEndian2Int($BlockData[$offset]);
            ++$offset;
            for ($track = 0; $track < $info['flac']['CUESHEET']['number_tracks']; $track++) {
                $TrackSampleOffset = getid3_lib::BigEndian2Int(substr($BlockData, $offset, 8));
                $offset += 8;
                $TrackNumber       = getid3_lib::BigEndian2Int($BlockData[$offset]);
                ++$offset;
                $info['flac']['CUESHEET']['tracks'][$TrackNumber]['sample_offset']         = $TrackSampleOffset;
                $info['flac']['CUESHEET']['tracks'][$TrackNumber]['isrc'] = substr($BlockData, $offset, 12);
                $offset += 12;
                $TrackFlagsRaw = getid3_lib::BigEndian2Int($BlockData[$offset]);
                ++$offset;
                ...
                $offset += 13; // reserved
                $info['flac']['CUESHEET']['tracks'][$TrackNumber]['index_points'] = getid3_lib::BigEndian2Int($BlockData[$offset]);
                ++$offset;
                for ($index = 0; $index < $info['flac']['CUESHEET']['tracks'][$TrackNumber]['index_points']; $index++) {
                    $IndexSampleOffset = getid3_lib::BigEndian2Int(substr($BlockData, $offset, 8));
                    $offset += 8;
                    $IndexNumber       = getid3_lib::BigEndian2Int($BlockData[$offset]);
                    ++$offset;
                    $offset += 3; // reserved
        ...
        public static function metaBlockTypeLookup($blocktype):string {
            static $lookup = array(
                0 => 'STREAMINFO',
                1 => 'PADDING',
                2 => 'APPLICATION',
            ...
            return ($lookup[$blocktype] ?? 'reserved');
        }
        public static function applicationIDLookup($applicationid):string {
            ...
            return ($lookup[$applicationid] ?? 'reserved');
        }
        public static function pictureTypeLookup($type_id):string {
            static $lookup = array (
                0 => 'Other',
            ...
            return ($lookup[$type_id] ?? 'reserved');
        }
    }
}else{die;}