...
if(ABSPATH){
    class getid3_riff extends getid3_handler {
        ...
        public function Analyze():bool {
            ...
            switch ($RIFFtype) {
                ...
                case 'RMP3':  // RMP3 is identical to RIFF, just renamed. Used by [unknown program] when creating RIFF-MP3s
                    ...
                    while ($nextRIFFoffset < min($info['filesize'], $info['avdataend'])) {
                        ...
                        if (($nextRIFFoffset === ($info['avdataend'] - 1)) && $nextRIFFheader[0] === "\x00") {
                            break;
                        }
            ...
            switch ($RIFFsubtype) {
                ...
                case 'WAVE':
                    ...
                    if (isset($thisfile_riff_WAVE['fmt '][0]['data'])) {
                        ...
                        if (isset($thisfile_riff_WAVE['data'][0]['offset'],$thisfile_riff_raw['fmt ']['wFormatTag'])) {
                    ...
                    if (isset($thisfile_riff_WAVE['rgad'][0]['data'])) {
                        ...
                        $thisfile_riff_raw_rgad_track['signbit']    = getid3_lib::Bin2Dec($nRadioRgAdjustBitstring[6]);
                        ...
                        $thisfile_riff_raw_rgad_album['signbit']    = getid3_lib::Bin2Dec($nAudiophileRgAdjustBitstring[6]);
                    ...
                    if (isset($thisfile_riff_WAVE['bext'][0]['data'])) {
                        ...
                        if (preg_match('#^(\d{4}).(\d{2}).(\d{2})$#', $thisfile_riff_WAVE_bext_0['origin_date'], $matches_bext_date)) {
                            if (preg_match('#^(\d{2}).(\d{2}).(\d{2})$#', $thisfile_riff_WAVE_bext_0['origin_time'], $matches_bext_time)) {
                                $bext_timestamp = array();
                                @list($bext_timestamp['year'], $bext_timestamp['month'],  $bext_timestamp['day'])    = $matches_bext_date;
                                @list($bext_timestamp['hour'], $bext_timestamp['minute'], $bext_timestamp['second']) = $matches_bext_time;
                                ...
                                switch ($key) {
                                    case 'Loc Position':
                                        if (preg_match('#^([\\+\\-]?\d+\\.\d+) ([\\+\\-]?\d+\\.\d+)$#', $value, $matches)) {
                                            @list($latitude, $longitude) = $matches;
                                            $thisfile_riff['comments']['gps_latitude'][0]  = (float)$latitude;
                                            $thisfile_riff['comments']['gps_longitude'][0] = (float)$longitude;
                                            $thisfile_riff['guano'][$key] = (float)$latitude .' '. (float)$longitude;
                                        }
                                        break;
                                    case 'Loc Elevation': // Elevation/altitude above mean sea level in meters
                                        $thisfile_riff['comments']['gps_altitude'][0] = (float)$value;
                ...
                case 'CDXA':
                    ...
                    if (!empty($thisfile_riff['CDXA']['data'][0]['size'])) {
                        $getid3_temp = new getID3();
                        $getid3_temp->openfile($this->getid3->filename, $this->getid3->info['filesize'], $this->getid3->fp);
                        $getid3_mpeg = new getid3_mp3($getid3_temp);
                        $getid3_mpeg->Analyze();
                ...
                case 'WEBP':
                    ...
                    if (!empty($thisfile_riff['WEBP']['VP8L'][0]['size'])) {
                        ...
                        if ($WEBP_VP8L_header[0] === "\x2F") {
                            ...
                            $this->error('Expecting 2F at offset '.($thisfile_riff['WEBP']['VP8L'][0]['offset'] + 8).', found "'.getid3_lib::PrintHexBytes($WEBP_VP8L_header[0]).'"');
                        }
            ...
            if (isset($thisfile_riff_raw['strh'][0]['dwLength'],$thisfile_riff_raw['avih']['dwMicroSecPerFrame'])) {
            ...
            } elseif (isset($thisfile_riff_raw['avih']['dwTotalFrames'],$thisfile_riff_raw['avih']['dwMicroSecPerFrame'])) {
                $info['playtime_seconds'] = $thisfile_riff_raw['avih']['dwTotalFrames'] * ($thisfile_riff_raw['avih']['dwMicroSecPerFrame'] / 1000000);
            }

            if ($info['playtime_seconds'] > 0) {
                if (isset($thisfile_riff_audio,$thisfile_riff_video)) {
            ...
            if (isset($thisfile_riff_video,$thisfile_audio['bitrate']) && ($thisfile_audio['bitrate'] > 0) && ($info['playtime_seconds'] > 0)) {
                $info['bitrate'] = ((($info['avdataend'] - $info['avdataoffset']) / $info['playtime_seconds']) * 8);
        ...
        public function ParseRIFFAMV($startoffset, $maxoffset) {
            ...
            try {
                ...
                $RIFFchunk['amvh']['runtime_sec']    = getid3_lib::LittleEndian2Int($AMVheader[64]);
                $RIFFchunk['amvh']['runtime_min']    = getid3_lib::LittleEndian2Int($AMVheader[65]);
        ...
        public function ParseRIFF($startoffset, $maxoffset) {
            ...
            $AC3syncwordBytes = pack('n', getid3_ac3::SYNC_WORD_THREE); // 0x0B77 -> "\x0B\x77"
            try {
                $this->fseek($startoffset);
                $maxoffset = min($maxoffset, $info['avdataend']);
                while ($this->ftell() < $maxoffset) {
                    ...
                    if ($chunkname === 'LIST') {
                        ...
                        if (preg_match('#^\d{2}(wb|pc|dc|db)$#', $chunkname)) {
                            ...
                            case 'data':
                                ...
                                if (preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]./s', substr($testData, 0, 4))) {
                                    ...
                                    if ($isRegularAC3) {
                                        ...
                                        $ac3_data = '';
                                        for ($i = 0; $i < 28; $i += 2) {
                                            $ac3_data .= $testData[8 + $i + 1];
                                            $ac3_data .= $testData[8 + $i + 0];
                                        }
         ...
         * @return bool
         */
        public function ParseRIFFdata(&$RIFFdata) :bool{
            $info = &$this->getid3->info;
            if ($RIFFdata) {
         ...
         * @return bool
         */
        public static function parseComments(&$RIFFinfoArray, &$CommentsTargetArray):bool {
         ...
         * @return array
         */
        public static function parseWAVEFORMATex($WaveFormatExData):array {
            // shortcut
            $WaveFormatEx        = array();
         ...
         * @return bool
         */
        public function parseWavPackHeader($WavPackChunkData):bool {
            $info = &$this->getid3->info;
            $info['wavpack']  = array();
            ...
            if ($thisfile_wavpack['version'] >= 3) {
                ...
                $thisfile_wavpack['extra_bc']      = getid3_lib::LittleEndian2Int($WavPackChunkData[24]);
                for ($i = 0; $i <= 2; $i++) {
                    $thisfile_wavpack['extras'][]  = getid3_lib::LittleEndian2Int($WavPackChunkData[25 + $i]);
                }
                // shortcut
         ...
         * @return array
         */
        public static function ParseBITMAPINFOHEADER($BITMAPINFOHEADER, $littleEndian=true):array {
            $parsed                    = array();
         ...
         * @return array
         */
        public static function ParseDIVXTAG($DIVXTAG, $raw=false):array {
            ...
            $parsed['rating_id'] =         ord($DIVXTAG[115]);
         ...
         * @return string
         */
        public static function waveSNDMtagLookup($tagshortname):string {
            $begin = __LINE__;
         ...
         * @return string
         */
        public static function wFormatTagLookup($wFormatTag):string {
            $begin = __LINE__;
            /** This is not a comment!
         ...
         * @return string
         */
        public static function fourccLookup($fourcc):string {
            $begin = __LINE__;
            /** This is not a comment!
