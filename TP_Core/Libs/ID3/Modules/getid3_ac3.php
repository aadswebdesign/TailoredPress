...
if(ABSPATH){
    class getid3_ac3 extends getid3_handler{
        ...
        private $BSIoffset = 0;
        public const SYNC_WORD_THREE = 0x0B77;
        public function Analyze():bool {
            ...
            $thisfile_ac3_raw_bsi['bsid'] = (getid3_lib::LittleEndian2Int($tempAC3header[5]) & 0xF8) >> 3; // AC3 and E-AC3 put the "bsid" version identifier in the same place, but unfortnately the 4 bytes between the syncword and the version identifier are interpreted differently, so grab it here so the following code structure can make sense
            unset($tempAC3header);
            if (($this->AC3header['syncinfo'] !== self::SYNC_WORD_THREE) && !$this->isDependencyFor('matroska')) {
                unset($info['fileformat'], $info['ac3']);
                return $this->error('Expecting "'.dechex(self::SYNC_WORD_THREE).'" at offset '.$info['avdataoffset'].', found "'.dechex($this->AC3header['syncinfo']).'"');
            }
                        ...
                        if ($thisfile_ac3_raw_bsi['mixdef'] === 1) { // mixing option 2
                            ...
                            if ($thisfile_ac3_raw_bsi['flags']['mixdata3']) {
                                ...
                                if ($thisfile_ac3_raw_bsi['flags']['addspchdat']) {
                                    ...
                                    $thisfile_ac3_raw_bsi['flags']['addspchdat1'] = (bool) $this->readHeaderBSI(1); ++$mixdefbitsread;
            ...
            if (isset($thisfile_ac3_raw_bsi['bsmod'],$thisfile_ac3_raw_bsi['acmod'])) {
         ...
         * @return int
         */
        private function readHeaderBSI($length):int {
        ...
        public static function sampleRateCodeLookup2($fscod2) {
            ...
            return ($sampleRateCodeLookup2[$fscod2] ?? false);
        }
         ...
         * @return array
         */
        public static function channelsEnabledLookup($acmod, $lfeon):array {
            $lookup = array(
                'ch1'=>($acmod === 0),
                'ch2'=>($acmod === 0),
                'left'=>($acmod > 1),
        ...
        public static function blocksPerSyncFrame($numblkscod) {
            ...
            return ($blocksPerSyncFrameLookup[$numblkscod] ?? false);
        }
    }
}else{die;}