...
if(ABSPATH){
    class getid3_mp3 extends getid3_handler{
        ...
        public function Analyze():bool {
            ...
            if ($this->allow_bruteforce && !$this->getOnlyMPEGaudioInfo($info['avdataoffset'])) {
            ...
            if (!isset($info['playtime_seconds']) && isset($info['audio']['bitrate']) && ($info['audio']['bitrate'] > 0)) {
                $xingVBRheaderFrameLength = ((isset($info['mpeg']['audio']['VBR_frames']) && ($info['mpeg']['audio']['framelength'] ?? 0)));
        ...
        public function GuessEncoderOptions():string {
            ...
            } elseif (!empty($thisfile_mpeg_audio_lame['preset_used']) && isset($thisfile_mpeg_audio_lame['preset_used_id']) && (!in_array($thisfile_mpeg_audio_lame['preset_used_id'], $NamedPresetBitrates, true))) {
                    ...
                    case '--r3mix':
                        ...
                        switch ($ExplodedOptions[1]) {
                            ...
                            case 'r3mix':
                                ...
                                $_expected_lowpass = $ExpectedLowpass[$ExplodedOptions[1].'|'.$thisfile_mpeg_audio_lame['lowpass_frequency']];
                                if (!isset($_expected_lowpass) && ($thisfile_mpeg_audio_lame['lowpass_frequency'] < 22050) && (round($thisfile_mpeg_audio_lame['lowpass_frequency'] / 1000) < round($thisfile_mpeg_audio['sample_rate'] / 2000))) {
                                    ...
                                    switch ($ExplodedOptions[1]) {
                                        ...
                                        default:
                                            static $ExpectedResampledRate = array(
                                                ...
                                                'fm|32000'                => 32000, // 3.90
                                                'voice|32000'             => 32000);
                                            $_expected_resampled_rate = $ExpectedResampledRate[$ExplodedOptions[1].'|'.$thisfile_mpeg_audio['sample_rate']];
                                            if (!isset($_expected_resampled_rate)) {
                                                $encoder_options .= ' --resample '.$thisfile_mpeg_audio['sample_rate'];
                                            }
                                            break;
                                    }
         ...
         * @return bool
         */
        public function decodeMPEGaudioHeader($offset, &$info, $recursivesearch=true, $ScanAsCBR=false, $FastMPEGheaderScan=false):bool {
                ...
                switch ($thisfile_mpeg_audio['channelmode']) {
                    ...
                    case 'dual channel':
                        if (($thisfile_mpeg_audio['bitrate'] === 'free') || ($thisfile_mpeg_audio['bitrate'] === 64000) || ($thisfile_mpeg_audio['bitrate'] >= 96000)) {
                            ...
                            $this->error((int)round($thisfile_mpeg_audio['bitrate'] / 1000) .'kbps not allowed in Layer 2, '.$thisfile_mpeg_audio['channelmode'].'.');
                            return false;
                        }
                        break;
                ...
                if ((substr($headerstring, $VBRidOffset, strlen('Xing')) === 'Xing') || (substr($headerstring, $VBRidOffset, strlen('Info')) === 'Info')) {
                    ...
                    if (!empty($thisfile_mpeg_audio['VBR_frames'])) {
                        ...
                        if (!empty($thisfile_mpeg_audio['VBR_bytes'])) {
                            ...
                            $used_filesize -= (isset($info['id3v2']['headerlength']) ? (int)$info['id3v2']['headerlength'] : 0);
                    ...
                    if (substr($headerstring, $VBRidOffset + 120, 4) === 'LAME') {
                        ...
                        if ($thisfile_mpeg_audio_lame['numeric_version'] !== '') {
                            foreach (explode('.', $thisfile_mpeg_audio_lame['numeric_version']) as $key => $number) {
                                $thisfile_mpeg_audio_lame['integer_version'][$key] = (int)$number;
                            }
                            ...
                            if ((($thisfile_mpeg_audio_lame['integer_version'][0] * 1000) + $thisfile_mpeg_audio_lame['integer_version'][1]) >= 3090) { // cannot use string version compare, may have "LAME3.90" or "LAME3.100" -- see https://github.com/JamesHeinrich/getID3/issues/207
                                ...
                                $thisfile_mpeg_audio_lame['vbr_quality'] = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0x9B]);

                                // bytes $9C-$A4  Encoder short VersionString
                                $thisfile_mpeg_audio_lame['short_version'] = substr($headerstring, $LAMEtagOffsetContant + 0x9C, 9);

                                // byte $A5  Info Tag revision + VBR method
                                $LAMEtagRevisionVBRmethod = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0xA5]);
                                ...
                                $thisfile_mpeg_audio_lame['lowpass_frequency'] = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0xA6]) * 100;
                                ...
                                $EncodingFlagsATHtype = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0xAF]);
                                ...
                                $thisfile_mpeg_audio_lame['raw']['abrbitrate_minbitrate'] = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0xB0]);
                                ...
                                // byte $B4  Misc
                                $MiscByte = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0xB4]);
                                ...
                                // byte $B5  MP3 Gain
                                $thisfile_mpeg_audio_lame_raw['mp3_gain'] = getid3_lib::BigEndian2Int($headerstring[$LAMEtagOffsetContant + 0xB5], false, true);
            ...
            if (($ExpectedNumberOfAudioBytes > 0) && ($ExpectedNumberOfAudioBytes !== ($info['avdataend'] - $info['avdataoffset']))) {
                ...
                } else if ((($info['avdataend'] - $info['avdataoffset']) - $ExpectedNumberOfAudioBytes) === 1) {
                    $info['avdataend']--;
                } else {
         ...
         * @return bool
         */
        public function RecursiveFrameScanning(&$offset, &$nextframetestoffset, $ScanAsCBR):bool {
            $info = &$this->getid3->info;
            ...
            for ($i = 0; $i < $this->mp3_valid_check_frames; $i++) {
                ...
                if ($this->decodeMPEGaudioHeader($nextframetestoffset, $nextframetestarray, false)) {
                    ...
                    if ($ScanAsCBR) {
                        ...
                        if (!isset($nextframetestarray['mpeg']['audio']['bitrate'],$firstframetestarray['mpeg']['audio']['bitrate']) || ($nextframetestarray['mpeg']['audio']['bitrate'] !== $firstframetestarray['mpeg']['audio']['bitrate'])) {
                            return false;
                        }
                    }
                    ...
                    if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
                        ...
                        if ($next4[0] === "\xFF") {
                            ...
                            if ($MPEGaudioHeaderValidCache[$next4]) {
                                ...
                                $Distribution['padding'][(int)$LongMPEGpaddingLookup[$head4]] = isset($Distribution['padding'][(int)$LongMPEGpaddingLookup[$head4]]) ? ++$Distribution['padding'][(int)$LongMPEGpaddingLookup[$head4]] : 1;
         ...
         * @return bool
         */
        public function getOnlyMPEGaudioInfo($avdataoffset, $BitrateHistogram=false):bool {
            ...
            while ($SynchSeekOffset < $sync_seek_buffer_size) {
                ...
                if (($header[$SynchSeekOffset] === "\xFF") && ($header[($SynchSeekOffset + 1)] > "\xE0")) { // possible synch detected
                    ...
                    if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
                        ...
                        if (isset($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode']) && ($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode'] === 'vbr') && !(abs($info['audio']['bitrate'] - $FirstFrameThisfileInfo['audio']['bitrate']) <= 1)) {
                        ...
                        if ($BitrateHistogram) {
                            ...
                            $this->fseek($info[$synchstartoffset]);
         ...
         * @return array
         */
        public static function MPEGaudioVersionArray():array {
         ...
         * @return array
         */
        public static function MPEGaudioLayerArray():array {
         ...
         * @return array
         */
        public static function MPEGaudioBitrateArray():array {
         ...
         * @return array
         */
        public static function MPEGaudioFrequencyArray():array {
         ...
         * @return array
         */
        public static function MPEGaudioChannelModeArray():array {
         ...
         * @return array
         */
        public static function MPEGaudioModeExtensionArray():array {
         ...
         * @return array
         */
        public static function MPEGaudioEmphasisArray():array {
         ...
         * @return bool
         */
        public static function MPEGaudioHeaderBytesValid($head4, $allowBitrate15=false):bool {
         ...
         * @return bool
         */
        public static function MPEGaudioHeaderValid($rawarray, $echoerrors=false, $allowBitrate15=false):bool {
         ...
         * @return int
         */
        public static function XingVBRidOffset($version, $channelmode):int {
         ...
         * @return string
         */
        public static function LAMEvbrMethodLookup($VBRmethodID):string {
         ...
         * @return string
         */
        public static function LAMEmiscStereoModeLookup($StereoModeID):string {
         ...
         * @return string
         */
        public static function LAMEmiscSourceSampleFrequencyLookup($SourceSampleFrequencyID):string {
         ...
         * @return string
         */
        public static function LAMEsurroundInfoLookup($SurroundInfoID):string {
         ...
         * @return string
         */
        public static function LAMEpresetUsedLookup($LAMEtag):string {
