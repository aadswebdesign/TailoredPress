    ...
    class getid3_apetag extends getid3_handler{
        ...
        public function Analyze():bool {
            ...
            for ($i = 0; $i < $thisfile_ape['footer']['raw']['tag_items']; $i++) {
                ...
                $offset += 4;
                if (strpos(substr($APEtagData, $offset), "\x00") === false) {
                ...
                switch (strtolower($item_key)) {
                    ...
                    case 'mp3gain_undo':
                        if (preg_match('#^[\\-\\+]\d{3},[\\-\\+]\d{3},[NW]$#', $thisfile_ape_items_current['data'][0])) {
                        ...
                        break;
                    case 'mp3gain_minmax':
                        if (preg_match('#^\d{3},\d{3}$#', $thisfile_ape_items_current['data'][0])) {
        ...
        public function APEcontentTypeFlagLookup($contenttypeid) {
            static $APEcontentTypeFlagLookup = [0 => 'utf-8',1 => 'binary',2 => 'external', 3 => 'reserved'];
            return ($APEcontentTypeFlagLookup[$contenttypeid] ?? 'invalid');
        }
        public function APEtagItemIsUTF8Lookup($itemkey):bool {
            static $APEtagItemIsUTF8Lookup = ['title','subtitle','artist','album','debut album','publisher',
                'conductor','track','composer','comment','copyright','publicationright','file','year','record date',
                'record location','genre','media','related','isrc','abstract','language','bibliography'];
