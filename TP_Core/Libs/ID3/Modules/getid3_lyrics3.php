    ...
    class getid3_lyrics3 extends getid3_handler{
        public function Analyze():bool {
            ...
            if ($lyrics3end === 'LYRICSEND') {
                ...
                $lyrics3offset  = $info['filesize'] - $lyrics3size;
                $lyrics3version = 2;
            } else if (isset($info['ape']['tag_offset_start']) && ($info['ape']['tag_offset_start'] > 15)) {
        ...
        public function getLyrics3Data($endoffset, $version, $length):bool {
            $info = &$this->getid3->info;
        ...
        public function Lyrics3Timestamp2Seconds($rawtimestamp) {
            if (preg_match('#^\\[(\d{2}):(\d{2})\\]$#', $rawtimestamp, $regs)) {
                return (int) (($regs[1] * 60) + $regs[2]);
            }
            return false;
        }
        public function Lyrics3LyricsTimestampParse(&$Lyrics3data):bool {
            $lyricsarray = explode("\r\n", $Lyrics3data['raw']['LYR']);
            $notimestamplyricsarray = [];
            foreach ($lyricsarray as $key => $lyricline) {
                $regs = [];
                unset($thislinetimestamps);
                while (preg_match('#^(\\[\d{2}:\d{2}\\])#', $lyricline, $regs)) {
         ...
         * @return bool|null
         */
        public function IntString2Bool($char):bool {
            if ($char === '1') {
                return true;
            }
            if ($char === '0') {
