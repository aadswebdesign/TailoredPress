    ...
    class getid3_quicktime extends getid3_handler{
        ...
        public function Analyze():bool {
            ...
            if (!empty($info['quicktime']['comments']['location.ISO6709'])) {
                ...
                foreach ((array)$info['quicktime']['comments']['location.ISO6709'] as $ISO6709string) {
                    $ISO6709parsed = array('latitude'=>false, 'longitude'=>false, 'altitude'=>false);
                    if (preg_match('#^([\\+\\-])(\d{2}|\d{4}|\d{6})(\\.\d+)?([\\+\\-])(\d{3}|\d{5}|\d{7})(\\.\d+)?(([\\+\\-])(\d{3}|\d{5}|\d{7})(\\.\d+)?)?/$#', $ISO6709string, $matches)) {
                        // phpcs:ignore PHPCompatibility.Lists.AssignmentOrder.Affected
                        @list($lat_sign, $lat_deg, $lat_deg_dec, $lon_sign, $lon_deg, $lon_deg_dec, $alt_sign, $alt_deg, $alt_deg_dec) = $matches;
                        if (strlen($lat_deg) === 2) {        // [+-]DD.D
                            $ISO6709parsed['latitude'] = (($lat_sign === '-') ? -1 : 1) * (float)(ltrim($lat_deg, '0') . $lat_deg_dec);
                        } elseif (strlen($lat_deg) === 4) {  // [+-]DDMM.M
                            $ISO6709parsed['latitude'] = (($lat_sign === '-') ? -1 : 1) * (float)ltrim(substr($lat_deg, 0, 2), '0') + (float)(ltrim(substr($lat_deg, 2, 2), '0') . $lat_deg_dec / 60);
                        } elseif (strlen($lat_deg) === 6) {  // [+-]DDMMSS.S
                            $ISO6709parsed['latitude'] = (($lat_sign === '-') ? -1 : 1) * (float)ltrim(substr($lat_deg, 0, 2), '0') + (float)(ltrim(substr($lat_deg, 2, 2), '0') / 60) + (float)(ltrim(substr($lat_deg, 4, 2), '0') . $lat_deg_dec / 3600);
                        }
                        if (strlen($lon_deg) === 3) {        // [+-]DDD.D
                            ...
                            $ISO6709parsed['longitude'] = (($lon_sign === '-') ? -1 : 1) * (float)ltrim(substr($lon_deg, 0, 2), '0') + (float)(ltrim(substr($lon_deg, 2, 2), '0') . $lon_deg_dec / 60);
                        } elseif (strlen($lon_deg) === 7) {  // [+-]DDDMMSS.S
                            $ISO6709parsed['longitude'] = (($lon_sign === '-') ? -1 : 1) * (float)ltrim(substr($lon_deg, 0, 2), '0') + (float)(ltrim(substr($lon_deg, 2, 2), '0') / 60) + (float)(ltrim(substr($lon_deg, 4, 2), '0') . $lon_deg_dec / 3600);
                        }
                        if (strlen($alt_deg) === 3) {        // [+-]DDD.D
                            ...
                            $ISO6709parsed['altitude'] = (($alt_sign === '-') ? -1 : 1) * (float)ltrim(substr($alt_deg, 0, 2), '0') + (float)(ltrim(substr($alt_deg, 2, 2), '0') . $alt_deg_dec / 60);
                        } elseif (strlen($alt_deg) === 7) {  // [+-]DDDMMSS.S
                            $ISO6709parsed['altitude'] = (($alt_sign === '-') ? -1 : 1) * (float)ltrim(substr($alt_deg, 0, 2), '0') + (float)(ltrim(substr($alt_deg, 2, 2), '0') / 60) + (float)(ltrim(substr($alt_deg, 4, 2), '0') . $alt_deg_dec / 3600);
                        }
                        foreach (array('latitude', 'longitude', 'altitude') as $key) {
                            if ($ISO6709parsed[$key] !== false) {
                                $value = (($lat_sign === '-') ? -1 : 1) * (float)$ISO6709parsed[$key];
                                $_info_quick_time = $info['quicktime']['comments']['gps_'.$key];
                                if (!isset($_info_quick_time) || !in_array($value, $info['quicktime']['comments']['gps_'.$key], true)) {
                                    @$info['quicktime']['comments']['gps_'.$key][] = (($lat_sign === '-') ? -1 : 1) * (float)$ISO6709parsed[$key];
                                }
                            }
                        }
                    }
        ...
        public function QuicktimeParseAtom($atomname, $atomsize, $atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
            ...
            if (strpos($atomname, "\x00\x00\x00") === 0) {
                ...
                $atomname = (string)getid3_lib::BigEndian2Int($atomname);
                $atom_structure['name'] = $atomname;
                ...
                switch ($atomname) {
                        ...
                        foreach ($atom_structure['subatoms'] as $key => $value_array) {
                            if (isset($value_array['sample_description_table'])) {
                                ...
                                foreach ($value_array['time_to_sample_table'] as $key2 => $value_array2) {
                                    if (isset($value_array2['sample_count'],$value_array2['sample_duration']) && ($value_array2['sample_duration'] > 0)) {
                    ...
                    case 'tvsn': // TV SeasoN
                        if ($atom_parent === 'udta') {
                            ...
                            if (empty($info['comments']['language']) || (!in_array($atom_structure['language'], $info['comments']['language'],true))) {
                                ...
                                while ($atomoffset < strlen($atom_data)) {
                                    ...
                                    switch ($boxtype) {
                                        ...
                                        case 'name':
                                            ...
                                            break;
                                        case 'data':
                                            $atom_structure['version']   = getid3_lib::BigEndian2Int($boxdata[0]);
                                                ...
                                                case 21: // tmpo/cpil flag
                                                    switch ($atomname) {
                                                        ...
                                                        case 'pgap':
                                                            ...
                                                            $atom_structure['data'] = getid3_lib::BigEndian2Int($boxdata[8]);
                                                            break;
                                                        case 'tmpo':
                                                            // 16-bit integer
                                                        ...
                                                        case 'rtng':
                                                            // 8-bit integer
                                                            $atom_structure[$atomname] = getid3_lib::BigEndian2Int($boxdata[8]);
                    ...
                    case 'play': // auto-PLAY atom
                        $atom_structure['autoplay'] = (bool) getid3_lib::BigEndian2Int($atom_data[0]);
                    ...
                    case 'rdrf': // Reference movie Data ReFerence atom
                        $atom_structure['version']                = getid3_lib::BigEndian2Int($atom_data[0]);
                    ...
                    case 'ptv ': // Print To Video - defines a movie's full screen mode
                        ...
                        $atom_structure['slide_show_flag']   = getid3_lib::BigEndian2Int($atom_data[6]);
                        $atom_structure['play_on_open_flag'] = getid3_lib::BigEndian2Int($atom_data[7]);
                    ...
                    case 'stsd': // Sample Table Sample Description atom
                        ...
                        for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
                            ...
                            if ($atom_structure['sample_description_table'][$i]['encoder_vendor'] === "\x00\x00\x00\x00") {
                                // audio tracks
                            ...
                            } else if ($atom_structure['sample_description_table'][$i]['data_format'] === 'mp4s') {
                                $info['fileformat'] = 'mp4';
                            } else {
                                // video atom
                    ...
                    case 'dref': // Data REFerence atom
                        ...
                        for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
                            ...
                            $drefDataOffset += 4;
                            $atom_structure['data_references'][$i]['version'] = getid3_lib::BigEndian2Int($atom_data[$drefDataOffset]);
                            ++$drefDataOffset;
                    ...
                    case 'iods': // Initial Object DeScriptor atom
                        $offset = 0;
                        $atom_structure['version'] = getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        $atom_structure['flags_raw'] = getid3_lib::BigEndian2Int(substr($atom_data, $offset, 3));
                        $offset += 3;
                        $atom_structure['mp4_iod_tag'] =  getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        ...
                        $offset += 2;
                        $atom_structure['od_profile_level'] = getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        $atom_structure['scene_profile_level'] = getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        $atom_structure['audio_profile_id'] = getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        $atom_structure['video_profile_id'] = getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        $atom_structure['graphics_profile_level'] = getid3_lib::BigEndian2Int($atom_data[$offset]);
                        ++$offset;
                        ...
                        for ($i = 0; $i < $atom_structure['num_iods_tracks']; $i++) {
                            $atom_structure['track'][$i]['ES_ID_IncTag'] =       getid3_lib::BigEndian2Int($atom_data[$offset]);
                            ++$offset;
                    ...
                    case 'mdat': // Media DATa atom
                        ...
                        while (($mdat_offset < (strlen($atom_data) - 8))
                            ...
                            @list($chapter_string) = $chapter_matches; //not used $dummy, $chapter_string_length_hex,
                                ...
                                if (!empty($getid3_temp->info['mpeg'])) {
                                    ...
                                    if (isset($info['mpeg']['audio'])) {
                                        ...
                                        if (!empty($info['mpeg']['audio']['codec'])) {
                                            $info['audio']['codec'] = (!empty($info['mpeg']['audio']['encoder']) ? $info['mpeg']['audio']['encoder'] : ($info['mpeg']['audio']['codec']));
                                        } else if (!empty($info['mpeg']['audio']['LAME'])) {
                                            $info['audio']['codec'] = (!empty($info['mpeg']['audio']['encoder']) ? $info['mpeg']['audio']['encoder'] : (('LAME')));
                                        } else {
                                            $info['audio']['codec'] = (!empty($info['mpeg']['audio']['encoder']) ? $info['mpeg']['audio']['encoder'] : (('mp3')));
                                        }
                    ...
                    case "\xA9".'xyz':  // GPS latitude+longitude+altitude
                        $atom_structure['data'] = $atom_data;
                        if (preg_match('#([\\+\\-][0-9\\.]+)([\\+\\-][0-9\\.]+)([\\+\\-][0-9\\.]+)?/$#', $atom_data, $matches)) {
                            @list($latitude, $longitude, $altitude) = $matches;
                    ...
                    case 'NCM2': // Nikon Camera preview iMage 2
                        ...
                        if (preg_match('/^\xFF\xD8\xFF/', $atom_data)) {
                            ...
                            $atom_structure['data'] = $atom_data;
                            $atom_structure['image_mime'] = 'image/jpeg';
                            $atom_structure['description'] = $descriptions[$atomname] ?? 'Nikon preview image';
                        ...
                        if ($atom_data !== '') {
                            if ((strlen($atom_data) % $GPS_rowsize) === 0) {
                                ...
                                foreach ($atom_structure['gps_toc'] as $key => $gps_pointer) {
                                    ...
                                    if (preg_match('#\\$GPRMC,([0-9\\.]*),([AV]),([0-9\\.]*),([NS]),([0-9\\.]*),([EW]),([0-9\\.]*),([0-9\\.]*),(\d*),([0-9\\.]*),([EW]?)(,[A])?(\\*[0-9A-F]{2})#', $GPS_free_data, $matches)) {
                                        $GPS_this_GPRMC = [];
                                        $GPS_this_GPRMC_raw = [];
                                        @list(
                                        ...
                                        foreach (array('latitude','longitude') as $latlon) {
                                            preg_match('#^(\d{1,3})(\d{2}\\.\d+)$#', $GPS_this_GPRMC['raw'][$latlon], $matches);
                                            @list($deg, $min) = $matches;
                    ...
                    case 'loci':// 3GP location (El Loco)
                        ...
                        $info['quicktime']['comments']['gps_role']      = array(  getid3_lib::BigEndian2Int($loci_data[0]));
                    ...
                    case 'chpl': // CHaPter List
                        ...
                        //$chpl_version = getid3_lib::BigEndian2Int($atom_data[4]); // Expected to be 0
                        //$chpl_flags   = getid3_lib::BigEndian2Int(substr($atom_data, 5, 3)); // Reserved, set to 0
                        $chpl_count   = getid3_lib::BigEndian2Int($atom_data[8]);
                        $chpl_offset = 9;
                        for ($i = 0; $i < $chpl_count; $i++) {
                            ...
                            $chpl_offset += 8;
                            $chpl_title_size = getid3_lib::BigEndian2Int($atom_data[$chpl_offset]);
                            ++$chpl_offset;
         ...
         * @return int
         */
        public function quicktime_read_mp4_descr_length($data, &$offset):int {
         ...
         * @return string
         */
        public function QuicktimeLanguageLookup($languageid):string {
            ...
            if (empty($QuicktimeLanguageLookup)) {
                ...
                $QuicktimeLanguageLookup[29]    = 'Latvian';
                $QuicktimeLanguageLookup[30]    = 'Saamisk';
                $QuicktimeLanguageLookup[31]    = 'Lappish';
                $QuicktimeLanguageLookup[32]    = 'Faeroese';
                $QuicktimeLanguageLookup[33]    = 'Farsi';
                $QuicktimeLanguageLookup[34]    = 'Persian';
                $QuicktimeLanguageLookup[35]    = 'Russian';
                $QuicktimeLanguageLookup[36]    = 'Chinese (Simplified)';
                $QuicktimeLanguageLookup[37]    = 'Flemish';
                $QuicktimeLanguageLookup[38]    = 'Irish';
                $QuicktimeLanguageLookup[39]    = 'Albanian';
                $QuicktimeLanguageLookup[40]    = 'Romanian';
                $QuicktimeLanguageLookup[41]    = 'Czech';
                $QuicktimeLanguageLookup[42]    = 'Slovak';
                $QuicktimeLanguageLookup[43]    = 'Slovenian';
                $QuicktimeLanguageLookup[44]    = 'Yiddish';
                $QuicktimeLanguageLookup[45]    = 'Serbian';
                $QuicktimeLanguageLookup[46]    = 'Macedonian';
                $QuicktimeLanguageLookup[47]    = 'Bulgarian';
                $QuicktimeLanguageLookup[48]    = 'Ukrainian';
                $QuicktimeLanguageLookup[49]    = 'Byelorussian';
                $QuicktimeLanguageLookup[50]    = 'Uzbek';
                $QuicktimeLanguageLookup[51]    = 'Kazakh';
                $QuicktimeLanguageLookup[52]    = 'Azerbaijani';
                $QuicktimeLanguageLookup[53]    = 'AzerbaijanAr';
                $QuicktimeLanguageLookup[54]    = 'Armenian';
                $QuicktimeLanguageLookup[55]    = 'Georgian';
                $QuicktimeLanguageLookup[56]    = 'Moldavian';
                $QuicktimeLanguageLookup[57]    = 'Kirghiz';
                $QuicktimeLanguageLookup[58]    = 'Tajiki';
                $QuicktimeLanguageLookup[59]    = 'Turkmen';
                $QuicktimeLanguageLookup[60]    = 'Mongolian';
                $QuicktimeLanguageLookup[61]    = 'MongolianCyr';
                $QuicktimeLanguageLookup[62]    = 'Pashto';
                $QuicktimeLanguageLookup[63]    = 'Kurdish';
                $QuicktimeLanguageLookup[64]    = 'Kashmiri';
                $QuicktimeLanguageLookup[65]    = 'Sindhi';
                $QuicktimeLanguageLookup[66]    = 'Tibetan';
                $QuicktimeLanguageLookup[67]    = 'Nepali';
                $QuicktimeLanguageLookup[68]    = 'Sanskrit';
                $QuicktimeLanguageLookup[69]    = 'Marathi';
                $QuicktimeLanguageLookup[70]    = 'Bengali';
                $QuicktimeLanguageLookup[71]    = 'Assamese';
                $QuicktimeLanguageLookup[72]    = 'Gujarati';
                $QuicktimeLanguageLookup[73]    = 'Punjabi';
                $QuicktimeLanguageLookup[74]    = 'Oriya';
                $QuicktimeLanguageLookup[75]    = 'Malayalam';
                $QuicktimeLanguageLookup[76]    = 'Kannada';
                $QuicktimeLanguageLookup[77]    = 'Tamil';
                $QuicktimeLanguageLookup[78]    = 'Telugu';
                $QuicktimeLanguageLookup[79]    = 'Sinhalese';
                $QuicktimeLanguageLookup[80]    = 'Burmese';
                $QuicktimeLanguageLookup[81]    = 'Khmer';
                $QuicktimeLanguageLookup[82]    = 'Lao';
                $QuicktimeLanguageLookup[83]    = 'Vietnamese';
                $QuicktimeLanguageLookup[84]    = 'Indonesian';
                $QuicktimeLanguageLookup[85]    = 'Tagalog';
                $QuicktimeLanguageLookup[86]    = 'MalayRoman';
                $QuicktimeLanguageLookup[87]    = 'MalayArabic';
                $QuicktimeLanguageLookup[88]    = 'Amharic';
                $QuicktimeLanguageLookup[89]    = 'Tigrinya';
                $QuicktimeLanguageLookup[90]    = 'Galla';
                $QuicktimeLanguageLookup[91]    = 'Oromo';
                $QuicktimeLanguageLookup[92]    = 'Somali';
                $QuicktimeLanguageLookup[93]    = 'Swahili';
                $QuicktimeLanguageLookup[94]    = 'Ruanda';
                $QuicktimeLanguageLookup[95]    = 'Rundi';
                $QuicktimeLanguageLookup[96]    = 'Chewa';
                $QuicktimeLanguageLookup[97]    = 'Malagasy';
                $QuicktimeLanguageLookup[98]    = 'Esperanto';
         ...
         * @return string
         */
        public function QuicktimeVideoCodecLookup($codecid):string {
            ...
            if (empty($QuicktimeVideoCodecLookup)) {
                ...
                $QuicktimeVideoCodecLookup['SVQ3'] = 'Sorenson Video 3';
         ...
         * @return string
         */
        public function QuicktimeDCOMLookup($compressionid):string {
         ...
         * @return string
         */
        public function QuicktimeColorNameLookup($colordepthid):string {
         ...
         * @return string
         */
        public function QuicktimeSTIKLookup($stik):string {
         ...
         * @return string
         */
        public function QuicktimeIODSaudioProfileName($audio_profile_id):string {
         ...
         * @return string
         */
        public function QuicktimeIODSvideoProfileName($video_profile_id):string {
         ...
         * @return string
         */
        public function QuicktimeContentRatingLookup($rtng):string {
         ...
         * @return string
         */
        public function QuicktimeStoreAccountTypeLookup($akid):string {
         ...
         * @return string
         */
        public function QuicktimeStoreFrontCodeLookup($sfid):string {
         ...
         * @return bool
         */
        public function CopyToAppropriateCommentsSection($keyname, $data, $boxname=''):bool {
            ...
            if ($comment_key) {
                ...
                foreach ($gooddata as $data_2) {
                    if (!empty($info['quicktime']['comments'][$comment_key]) && in_array($data_2, $info['quicktime']['comments'][$comment_key], true)) {
                        // avoid duplicate copies of identical data
                        continue;
                    }
                    $info['quicktime']['comments'][$comment_key][] = $data_2;
                }
            }
            return true;
        }
         ...
         * @return string
         */
        public function LociString($lstring, &$count):string {
         ...
         * @return string
         */
        public function NoNullString($nullterminatedstring):string {
            // remove the single null terminator on null terminated strings
            if ($nullterminatedstring[strlen($nullterminatedstring) - 1] === "\x00") {
         ...
         * @return string
         */
        public function Pascal2String($pascalstring):string {
         ...
         * @return string
         */
        public function MaybePascal2String($pascalstring):string {
            // Pascal strings have 1 unsigned byte at the beginning saying how many chars (1-255) are in the string
            // Check if string actually is in this format or written incorrectly, straight string, or null-terminated string
            if (ord($pascalstring[0]) === (strlen($pascalstring) - 1)) {
                return substr($pascalstring, 1);
            }
            if ($pascalstring[strlen($pascalstring) - 1] === "\x00") {
         ...
         * @param array  $result
         */
        public function search_tag_by_key($info, $tag, $history, &$result):void {
         ...
         * @param array  $result
         */
        public function search_tag_by_pair($info, $k, $v, $history, &$result):void {
            foreach ($info as $key => $value) {
                $key_history = $history.'/'.$key;
                if (($key === $k) && ($value === $v)) {
                    $result[] = array($key_history, $info);
         ...
         * @return bool
         */
        public function quicktime_time_to_sample_table($info):bool {
            $res = array();
        ...
        public function quicktime_bookmark_time_scale($info):int {
            $time_scale = '';
            $ts_prefix_len = 0;
            $res = array();
            $this->search_tag_by_pair($info['quicktime']['moov'], 'name', 'stbl', 'quicktime/moov', $res);
