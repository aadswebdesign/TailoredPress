 ...
 * Time: 11:41
 */
namespace TP_Core\Libs\PHP\SodiumCompat\src\Core\Poly1305;
...
if(ABSPATH){
    class ParagonIE_Poly1305_State extends ParagonIE_Util{
        /**
         * @var array<int, int>
         */
        ...
        public function update($message = ''):self
        {
            ...
            /* handle leftover */
            if ($this->leftover) {
                $want = ParagonIE_Poly1305::BLOCK_SIZE - $this->leftover;
                if ($want > $bytes) {
                    $want = $bytes;
                }
                for ($i = 0; $i < $want; ++$i) {
                ...
                $bytes = self::strlen($message);
                $this->leftover += $want;
                if ($this->leftover < ParagonIE_Poly1305::BLOCK_SIZE) {
                ...
                $this->blocks(
                    ...
                    ParagonIE_Poly1305::BLOCK_SIZE
                );
                $this->leftover = 0;
            }

            /* process full blocks */
            if ($bytes >= ParagonIE_Poly1305::BLOCK_SIZE) {
                /** @var int $want */
                $want = $bytes & ~(ParagonIE_Poly1305::BLOCK_SIZE - 1);
                if ($want >= ParagonIE_Poly1305::BLOCK_SIZE) {
                    $block = self::substr($message, 0, $want);
                    if (self::strlen($block) >= ParagonIE_Poly1305::BLOCK_SIZE) {
        ...
        public function blocks($message, $bytes):self
        {
            ...
            while ($bytes >= ParagonIE_Poly1305::BLOCK_SIZE) {
                /* h += m[i] */
                ...
                $bytes -= ParagonIE_Poly1305::BLOCK_SIZE;
            }
        ...
        public function finish():string
        {
            ...
            if ($this->leftover) {
                $i = $this->leftover;
                $this->buffer[$i++] = 1;
                for (; $i < ParagonIE_Poly1305::BLOCK_SIZE; ++$i) {
                    $this->buffer[$i] = 0;
                }
                $this->final = true;
                $this->blocks(
