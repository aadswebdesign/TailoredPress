...
if(ABSPATH){
    class TreeBuildingRules{
        protected static $_tags = array(
            'li' => 1,'dd' => 1,'dt' => 1,'rt' => 1,'rp' => 1,'tr' => 1,'th' => 1,
            'td' => 1,'thead' => 1,'tfoot' => 1,'tbody' => 1,'table' => 1,'optgroup' => 1,'option' => 1,
        );
        public function hasRules($tag_name):bool{
            return isset(static::$_tags[$tag_name]);
        }
        ...
        protected function closeIfCurrentMatches($ele, \DOMDocument $current, $match){
            if (in_array($current->getElementsByTagName($ele), $match, true))
                $current->parentNode->appendChild($ele);
            else $current->appendChild($ele);
            return $ele;
        }
    }
}else die;