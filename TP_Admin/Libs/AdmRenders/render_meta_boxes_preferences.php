...
if(ABSPATH){
    class render_meta_boxes_preferences{
        ...
        private function __to_string(): string{
            ...
            $output = "<fieldset class='metabox-prefers'><legend>{$this->__('Screen elements')}</legend><ul>";
            $output .= "<li><p>{$content['p1']}</p></li>";
            ob_start();
            ...
            if ( 'dashboard' === $this->_args['meta_id'] && $this->_has_action( 'welcome_panel' ) && $this->_current_user_can( 'edit_theme_options' ) ) {
                ...
                $output .= "<li>";
                $output .= <dt><label for='tp_welcome_panel_hide'>{$this->__( 'Welcome', 'Welcome panel' )}</label></dt>
                $output .= "<dd><input type='checkbox' id='tp_welcome_panel_hide'  {$this->_get_checked( (bool) $welcome_checked, true )}/></dd>";
				$output .= "</li>\n";
            }
            $output .= "</ul></fieldset>";
